...
if (
  location.search.includes("success_msg") ||
  location.search.includes("error_msg")
) {
  history.replaceState &&
    history.replaceState(
      null,
  ...
  history.replaceState &&
    history.replaceState(
      ...
      "",
      location.pathname +
        location.search.replace(/[\?&]error_msg=[^&]+/, "").replace(/^&/, "?") +
        location.hash
    );
}
document.body.addEventListener("click", (event) => {
  if (!event.target.closest(".multiselect")) {
    document
      .querySelectorAll(".multiselect .list")
...
document.querySelectorAll(".multiselect").forEach((element) => {
    ...
    element.querySelectorAll(".item").forEach((item) => {
      ...
      item.querySelector(".remove").onclick = () => {
        element.querySelector(
          '.list span[data-value="' + item.dataset.value + '"]'
        ).style.display = "flex";
  ...
  element.querySelector(".search").onfocus = () =>
    (element.querySelector(".list").style.display = "flex");
  element.querySelector(".search").onclick = () =>
    (element.querySelector(".list").style.display = "flex");
  element.querySelector(".search").onkeyup = () => {
    ...
    element.querySelectorAll(".list span").forEach((item) => {
      item.style.display = item.innerText
        .toLowerCase()
        .includes(element.querySelector(".search").value.toLowerCase())
        ? "flex"
        : "none";
    });
    updateList();
  };
  element.querySelectorAll(".list span").forEach(
    (item) =>
      (item.onclick = () => {
...
const openMediaLibrary = (options) =>
  modal({
    media: [],
    state: "open",
        ...
        <div class="dialog large media-library-modal">
            <div class="content">
                ...
                <div class="footer pad-5">
                    <a href="#" class="btn dialog-close save">${
                      options.buttonText ? options.buttonText : "Save"
                    }</a>
                </div>
            </div>
        </div>
    ...
    detailsTemplate: function (media, img) {
      return `
        <form class="media-details-form" method="post" action="index.php?page=api&action=media&id=${
          media.id
        }">
            <h3>Media Details</h3>
            <a href="${img}" target="_blank"><img src="${img}" alt="${
        media.caption
      }"></a>
            <label for="title">Title</label>
            <input id="title" type="text" name="title" value="${media.title}">
            <label for="caption">Caption</label>
            <input id="caption" type="text" name="caption" value="${
              media.caption
            }">
            <label for="full_path">Full Path</label>
            <input id="full_path" type="text" name="full_path" value="${
              media.full_path
            }">
            <label for="date_uploaded">Date Uploaded</label>
            <input id="date_uploaded" type="datetime-local" name="date_uploaded" value="${media.date_uploaded.replace(
              " ",
              "T"
            )}">
    ...
    populateMedia: function (data) {
      ...
      if (this.media.length > 0) {
        document
          .querySelectorAll(".media-library-modal a.media-image")
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        a.onclick = (event) => {
          ...
          if (a.classList.contains("selected")) {
            ...
            ).innerHTML = this.detailsTemplate(data[i], img.src);
          } else if (
            document.querySelector(
              ".media-library-modal a.media-image.selected"
            )
          ) {
            this.unselectMedia(data[i].id);
            let selectedMedia = document.querySelector(
              ".media-library-modal a.media-image.selected"
            );
            document.querySelector(
              ".media-library-modal .media .details"
            ).innerHTML = this.detailsTemplate(
              data[selectedMedia.dataset.index],
              selectedMedia.querySelector("img").src
            );
          } else {
            this.unselectMedia(data[i].id);
            document.querySelector(
              ".media-library-modal .media .details"
            ).innerHTML = `<p>No media selected.</p>`;
          }
          document
            .querySelectorAll(".media-library-modal .media .details input")
            .forEach(
              (element) =>
                (element.onkeyup = () =>
                  (document.querySelector(
                    ".media-library-modal .save-media"
                  ).style.display = "inline-flex"))
            );
          if (document.querySelector(".media-library-modal .save-media")) {
            let mediaDetailsForm = document.querySelector(
              ".media-library-modal .media-details-form"
            );
            document.querySelector(".media-library-modal .save-media").onclick =
              (event) => {
                event.preventDefault();
            ...
            document.querySelector(
              ".media-library-modal .delete-media"
            ).onclick = (event) => {
              event.preventDefault();
              if (confirm("Are you sure you want to delete this media?")) {
                fetch(mediaDetailsForm.action + "&delete=true")
                  .then((response) => response.json())
                  .then((newData) => {
                    for (let j = 0; j < this.media.length; j++) {
                      for (let k = 0; k < newData.length; k++) {
                        if (
                          this.media[j].id == newData[k].id &&
                          this.media[j].selected
                        ) {
    ...
    onOpen: function () {
      fetch("index.php?page=api&action=media", { cache: "no-store" })
        .then((response) => response.json())
        .then((data) => {
          this.media = data;
          this.populateMedia();
      ...
      document.querySelector(".media-library-modal .upload-media").onclick = (
        event
      ) => {
        event.preventDefault();
        ...
        input.onchange = (event) => {
          document.querySelector(
            ".media-library-modal .upload-media"
            ...
            this.media.filter((media) => {
              return (
                media.title
                  ...
                  .includes(
                    document
                      .querySelector(".media-library-modal .search-media")
                      .value.toLowerCase()
                  ) ||
                media.caption
                  .toLowerCase()
    ...
    onClose: function (event) {
      if (
        ...
        event &&
        event.button &&
        event.button.classList.contains("save")
      )
        options.onSave(this.getAllSelectedMedia());
  ...
  modal({
      ...
      return `
        <div class="option-content" data-type="${type}">
            ...
            ${
              type == "text" || type == "datetime"
                ? ""
                : '<a href="#" class="add-option-value-btn"><i class="fa-solid fa-plus"></i>Add Option Value</a>'
            }
        </div>           
        `;
    },
    optionValueTemplate: function (type, option) {
      if (type == "text" || type == "datetime") {
        return `
            <tr class="option-value">
                <td>
                    ${
                      type == "text"
                        ? '<input type="text" placeholder="Value" class="name" value="' +
                          (option ? option.name : "") +
                          '">'
                        : '<input type="datetime-local" class="name" value="' +
                          (option ? option.name : "") +
                          '">'
                    }
                ...
                <td>
                    <div class="input-group">
                        <select class="modifier price-mod">
                            <option value="add"${
                              option && option.price_modifier == "add"
                                ? " selected"
                                : ""
                            }>+</option>
                            <option value="subtract"${
                              option && option.price_modifier == "subtract"
                                ? " selected"
                                : ""
                            }>-</option>
                        </select>
                        <input type="number" class="price" placeholder="Price" min="0" step=".01" value="${
                          option ? option.price : ""
                        }">
                    </div>
                </td>
                <td>
                    <div class="input-group">
                        <select class="modifier weight-mod">
                            <option value="add"${
                              option && option.weight_modifier == "add"
                                ? " selected"
                                : ""
                            }>+</option>
                            <option value="subtract"${
                              option && option.weight_modifier == "subtract"
                                ? " selected"
                                : ""
                            }>-</option>
                        </select>
                        <input type="number" class="weight" placeholder="Weight" min="0" value="${
        ...
        return `
            <tr class="option-value">
                <td><input type="text" placeholder="Name" class="name" value="${
                  option ? option.name : ""
                }"></td>
                <td><input type="number" placeholder="Quantity" class="quantity" title="-1 = unlimited" value="${
    ...
    onOpen: function () {
      this.element.querySelector(".option-type").onchange = () => {
        this.element
          ...
          .forEach((element) => (element.style.display = "none"));
        this.element.querySelectorAll(".option-content")[
          this.element.querySelector(".option-type").value
        ].style.display = "flex";
        ...
        if (
          this.selectedOptionContainer.querySelector(".add-option-value-btn")
        ) {
          this.selectedOptionContainer.querySelector(
            ".add-option-value-btn"
          ).onclick = (event) => {
            event.preventDefault();
            this.selectedOptionContainer
              .querySelector("tbody")
              .insertAdjacentHTML("beforeend", this.optionValueTemplate());
            this.element
              .querySelectorAll(".remove-option-value")
              .forEach(
                (element) =>
                  (element.onclick = () =>
                    element.closest(".option-value").remove())
              );
          };
        }
      };
      if (options.options && options.options.length > 0) {
        this.element.querySelector(".option-title").value =
          options.options[0].title;
        this.element.querySelector(".option-title").dataset.defaultTitle =
          options.options[0].title;
        this.element.querySelector(".option-required").checked = parseInt(
          options.options[0].required
        )
          ? true
          : false;
        this.element.querySelector(".option-type").value =
        ...
        options.options.forEach((option) => {
          this.selectedOptionContainer
            ...
            .insertAdjacentHTML(
              "beforeend",
              this.optionValueTemplate(option.type, option)
            );
        });
        this.element
          .querySelectorAll(".remove-option-value")
      ...
      if (
        ...
        if (!this.element.querySelector(".option-title").value) {
          this.element
            ...
            .setCustomValidity("Please enter the option title!");
          this.element.querySelector(".option-title").reportValidity();
          return false;
        }
        if (
          options.reservedTitles.includes(
            this.element.querySelector(".option-title").value.toLowerCase()
          ) &&
          this.element.querySelector(".option-title").value.toLowerCase() !=
            this.element
              .querySelector(".option-title")
              .dataset.defaultTitle.toLowerCase()
        ) {
          this.element
            .querySelector(".option-title")
            .setCustomValidity("Title already exists!");
          this.element.querySelector(".option-title").reportValidity();
          return false;
        }
        let productOptions = [];
        this.selectedOptionContainer
          ...
          .forEach((optionValue) => {
            productOptions.push({
              ...
              required: this.element.querySelector(".option-required").checked
                ? 1
                : 0,
            });
          });
...
const initProduct = () => {
  let productMediaHandler = () => {
    document.querySelectorAll(".media-position i").forEach(
      (element) =>
        (element.onclick = (event) => {
          event.preventDefault();
          let mediaElement = element.closest(".product-media");
          if (
            element.classList.contains("move-up") &&
            mediaElement.previousElementSibling
          ) {
            mediaElement.parentNode.insertBefore(
              mediaElement,
              mediaElement.previousElementSibling
            );
          }
          if (
            ...
            mediaElement.nextElementSibling
          ) {
            mediaElement.parentNode.insertBefore(
              mediaElement.nextElementSibling,
              mediaElement
          ...
          if (
            element.classList.contains("media-delete") &&
    ...
    openMediaLibrary({
      ...
      onSave: function (media) {
        ...
        media.forEach((m) => {
          ...
          document.querySelector(".product-media-container").insertAdjacentHTML(
            "beforeend",
            `
                        <div class="product-media">
                            <span class="media-index responsive-hidden">${
                              index + 1
                            }</span>
                            ...
                            <input type="hidden" class="input-media-id" name="media[]" value="${
                              m.id
                            }">
                            <input type="hidden" class="input-media-product-id" name="media_product_id[]" value="0">
                            <input type="hidden" class="input-media-position" name="media_position[]" value="${
                              index + 1
                            }">
                        </div>                    
                    `
  ...
  let productOptionsHandler = () => {
    document.querySelectorAll(".option-position i").forEach(
      (element) =>
        (element.onclick = (event) => {
          event.preventDefault();
          ...
          if (
            element.classList.contains("move-up") &&
            optionElement.previousElementSibling
          ) {
            optionElement.parentNode.insertBefore(
              optionElement,
              optionElement.previousElementSibling
            );
          }
          if (
            element.classList.contains("move-down") &&
            optionElement.nextElementSibling
          ) {
            optionElement.parentNode.insertBefore(
              optionElement.nextElementSibling,
              optionElement
          ...
          if (
            element.classList.contains("option-delete") &&
          ...
          if (element.classList.contains("option-edit")) {
            let options = [];
            optionElement
              .querySelectorAll(".input-option-value")
              .forEach((optionValue) => {
                options.push({
                  title: optionValue.querySelector(".input-option-title").value,
                  name: optionValue.querySelector(".input-option-name").value,
                  quantity: optionValue.querySelector(".input-option-quantity")
                    .value,
                  price: optionValue.querySelector(".input-option-price").value,
                  price_modifier: optionValue.querySelector(
                    ".input-option-price-modifier"
                  ).value,
                  weight: optionValue.querySelector(".input-option-weight")
                    .value,
                  weight_modifier: optionValue.querySelector(
                    ".input-option-weight-modifier"
                  ).value,
                  type: optionValue.querySelector(".input-option-type").value,
                  required: optionValue.querySelector(".input-option-required")
                    .value,
                });
              });
            openOptionsModal({
              ...
              reservedTitles: [
                ...
                option.querySelector(".input-option-title").value.toLowerCase()
              ),
              options: options,
              onSave: function (options) {
                if (options.length > 0) {
                  ...
                  options.forEach((option) => {
                    optionsHtml += `
                                <div class="input-option-value">
                                    <input type="hidden" class="input-option-title" name="option_title[]" value="${
                                      option.title
                                    }">
                                    <input type="hidden" class="input-option-name" name="option_name[]" value="${
                                      option.name
                                    }">
                                    <input type="hidden" class="input-option-quantity" name="option_quantity[]" value="${
                                      option.quantity
                                    }">
                                    <input type="hidden" class="input-option-price" name="option_price[]" value="${
                                      option.price
                                    }">
                                    <input type="hidden" class="input-option-price-modifier" name="option_price_modifier[]" value="${
                                      option.price_modifier
                                    }">
                                    <input type="hidden" class="input-option-weight" name="option_weight[]" value="${
                                      option.weight
                                    }">
                                    <input type="hidden" class="input-option-weight-modifier" name="option_weight_modifier[]" value="${
                                      option.weight_modifier
                                    }">
                                    <input type="hidden" class="input-option-type" name="option_type[]" value="${
                                      option.type
                                    }">
                                    <input type="hidden" class="input-option-required" name="option_required[]" value="${
                                      option.required
                                    }">
                                    <input type="hidden" class="input-option-position" name="option_position[]" value="${
                  ...
                  optionElement.innerHTML = `                    
                                <span class="option-index responsive-hidden">${
                                  optionElement.querySelector(".option-index")
                                    .innerHTML
                                }</span>
          ...
          document
            ...
            .forEach((element, index) => {
              ...
              element
                .querySelectorAll(".input-option-position")
                .forEach((input) => (input.value = index + 1));
            });
        })
    );
  };
  ...
  document.querySelector(".open-options-modal").onclick = (event) => {
    ...
    openOptionsModal({
      buttonText: "Add",
      reservedTitles: [...document.querySelectorAll(".product-option")].map(
        (option) =>
          option.querySelector(".input-option-title").value.toLowerCase()
      ),
      onSave: function (options) {
        if (options.length > 0) {
          ...
          document
            .querySelector(".product-options-container")
            .insertAdjacentHTML(
              "beforeend",
              `
  ...
  let productDownloadsHandler = () => {
    document.querySelectorAll(".download-position i").forEach(
      (element) =>
        (element.onclick = (event) => {
          event.preventDefault();
          ...
          if (
            element.classList.contains("move-up") &&
            downloadElement.previousElementSibling
          ) {
            downloadElement.parentNode.insertBefore(
              downloadElement,
              downloadElement.previousElementSibling
            );
          }
          if (
            element.classList.contains("move-down") &&
            downloadElement.nextElementSibling
          ) {
            downloadElement.parentNode.insertBefore(
              downloadElement.nextElementSibling,
              downloadElement
          ...
          if (
            element.classList.contains("download-delete") &&
            confirm("Are you sure you want to delete this digital download?")
          ) {
            downloadElement.remove();
          }
          document
            .querySelectorAll(".product-download")
            .forEach((element, index) => {
              element.querySelector(".download-index").innerHTML = index + 1;
              element.querySelector(".input-download-position").value =
                index + 1;
            });
        })
    );
    ...
    modal({
        ...
        return `
                <div class="dialog downloads-modal">
                    <div class="content">
                        ...
                        <div class="body">
                            <p>The file path must be relative to the Omega Shop root directory.</p>
      ...
      onOpen: function () {
        this.element.querySelector(".download-file-path").onchange = () => {
          fetch(
            "index.php?page=api&action=fileexists&path=" +
              this.element.querySelector(".download-file-path").value,
            { cache: "no-store" }
          )
            .then((response) => response.json())
            .then((data) => {
              this.element.querySelector(".download-result").innerHTML =
                data.result;
              if (data.result) {
      ...
      onClose: function (event) {
        if (event && event.button && event.button.classList.contains("save")) {
          ...
          document
            .querySelector(".product-downloads-container")
            .insertAdjacentHTML(
              "beforeend",
              `
                        <div class="product-download">
                            <span class="download-index responsive-hidden">${
                              index + 1
                            }</span>
                            ...
                            <input type="hidden" class="input-download-position" name="download_position[]" value="${
                              index + 1
                            }">
                        </div>                    
                    `
...
const initMedia = () => {
  let mediaHandler = () => {
    document.querySelectorAll(".media .image").forEach(
      (element) =>
        (element.onclick = (event) => {
          event.preventDefault();
          modal({
            ...
            onClose: function (event) {
              let mediaDetailsForm = this.element.querySelector(
                ".media-details-form"
              );
              if (
                event &&
              ...
              if (
                event &&
                event.button &&
                event.button.classList.contains("delete")
              ) {
                fetch(mediaDetailsForm.action + "&delete=true")
                  .then((response) => response.json())
                  .then(() => element.remove());
        ...
        .then((data) => {
          if (data) {
            data.forEach((media, index) => {
              ...
              document.querySelector(".media").insertAdjacentHTML(
                "afterbegin",
                `
                        <a href="#" class="image" data-id="${
                          media.id
                        }" data-title="${media.title}" data-caption="${
                  media.caption
                }" data-date-uploaded="${media.date_uploaded.replace(
                  " ",
                  "T"
                )}" data-full-path="${media.full_path}">
                            <img src="../${media.full_path}" alt="${
                  media.caption
                }" loading="lazy">
                        </a>
                        `
...
const initManageOrder = (products) => {
  document.querySelector(".add-item").onclick = (event) => {
    event.preventDefault();
    document.querySelector(".manage-order-table tbody").insertAdjacentHTML(
      "beforeend",
      `
        <tr>
            <td>
